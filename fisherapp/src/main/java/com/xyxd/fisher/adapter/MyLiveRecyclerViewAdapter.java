...
public class MyLiveRecyclerViewAdapter extends RecyclerView.Adapter<MyLiveRecyclerViewAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        ...
        holder.mTimeView.setText("直播时间: \n" + df.format(live.getCloudLive().getStartTime()));
        holder.mImageView.setScaleType(ImageView.ScaleType.CENTER_CROP);
        ImageLoader imageLoader = ImageLoader.getInstance();
        String path = holder.mItem.getCloudLive().getCoverImgUrl();
        ...
        holder.mTitleView.setText("  " + holder.mItem.getCloudLive().getActivityName());
        if(holder.mItem.getCloudLive().getActivityStatus() == 0)
            holder.mStatuView.setText("未开始");
        if(holder.mItem.getCloudLive().getActivityStatus() == 1) {
            holder.mStatuView.setText("直播中");
            holder.mStatuView.setBackgroundResource(R.color.colorPrimary);
        }
        if(holder.mItem.getCloudLive().getActivityStatus() == 2)
            holder.mStatuView.setText("已中断");
        if(holder.mItem.getCloudLive().getActivityStatus() == 3)

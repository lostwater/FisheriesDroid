...
public class MainActivity extends Activity {
	...
	private OnClickListener onClick = new OnClickListener() {
		...
		public void onClick(View v) {
			int i = v.getId();
			if (i == R.id.btn_no_push_stream) {
				addNoStreamView(v);

				// 展现有推流地址界面
			} else if (i == R.id.btn_push_stream) {
				addStreamView(v);

				// 展现乐视云直播界面
			} else if (i == R.id.btn_letv_stream) {
				addLetvView(v);

				// 开始直播 点击事件
			} else if (i == R.id.btn_submit_push) {
				pushStream();

				// 无推流地址中 生成播放地址 按钮点击事件
			} else if (i == R.id.btn_create_stream) {
				createPlayerUrl();
